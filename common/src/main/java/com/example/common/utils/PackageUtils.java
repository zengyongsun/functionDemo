...
public class PackageUtils {
     ...
     * @return true已装       false未装
     */
    public static boolean getMatchPacket(Context context, String packageName) {
    ...
    public static boolean isTopActivity(Context context, String activityName) {
        if (context == null || StringUtils.isEmpty(activityName)) {
            return false;
        }
        ...
        try {
            return tasksInfo.get(0).topActivity.getShortClassName().endsWith(activityName);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
