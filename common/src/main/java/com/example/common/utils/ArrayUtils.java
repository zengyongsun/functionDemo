...
public class ArrayUtils {
	 ...
	 * 根据分隔符将字符串分割并组装成List
	 * 
	 * @param listStr 为空时返回空ArrayList
	 * @param separator 为空时返回只含一个元素(listStr)的ArrayList
	 * @see {@link StringUtils}
	 * @return
	 */
	public static ArrayList<String> pareseFromString(String listStr,
                                                     String separator) {
		ArrayList<String> array = new ArrayList<String>();
		if (StringUtils.isEmpty(listStr) || StringUtils.isEmpty(separator)) {
			return array;
		}
		String[] splitArray = listStr.split(separator);
